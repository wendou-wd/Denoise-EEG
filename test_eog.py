...
if __name__ == '__main__':
    mix_EEG=np.load('./data/mix_EOG_test_6800.npy')
    print(mix_EEG)
    EEG=np.load('./data/true_test_6800.npy')
    ...
    epochpoint=0
    if os.path.exists('./train_EOG/gen/'):
    ...
    for i in range(0,10):
        ...
        test_mix_EEG_batch=test_mix_EEG[i*680:(i+1)*680,:]
        test_true_EEG_batch=test_true_EEG[i*680:(i+1)*680,:]
        print("ww",i*680,(i+1)*680)


        dataset=customData(test_mix_EEG_batch,test_true_EEG_batch)
        dataloader = DataLoader(dataset, batch_size=340,shuffle=False)
    ...
    # ax[0].plot(test_mix_EEG[173,:]*std_true,"b",label='NoiseEEG')
    # ax[0].plot(real[173,:],"r",label='Raw')
    # ax[0].plot(next[173,:],"g",label='Denoise')
    # ax[0].set_title('Worst Result')
    # ax[0].set_ylabel("Amplitude(uV)")
    # ax[0].legend(loc='lower right')
    # # ax[0].set_xlabel("Frequency(HZ)")
    #
    # ax[1].plot(test_mix_EEG[395,:]*std_true,"b",label='NoiseEEG')
    # ax[1].plot(real[395,:],"r",label='Raw')
    # ax[1].plot(next[395,:],"g",label='Denoise')
    # ax[1].set_title('Best Result')
    # ax[1].set_ylabel("Amplitude(uV)")
    # ax[1].legend(loc='lower right')
    # # ax[1].set_xlabel("Frequency(HZ)")
    # plt.show()
    fig.savefig("avgeog.tiff",dpi=600)
