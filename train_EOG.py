...
mix_EEG_col=np.load('./data/mix_EOG_27200.npy')
real_EEG_col=np.load('./data/true_27200.npy')
...
"""
save_root='./train_EOG'
dir_gen='./train_EOG/gen'
dir_disc='./train_EOG/disc'
epochpoint=0
if os.path.exists('./train_EOG/gen'):
    all = os.listdir(dir_gen)
    if len(all)>0:
        checkpoint=torch.load('./train_EOG/gen/'+all[len(all)-1])
...
if os.path.exists(dir_disc):
    ...
    if len(all)>0:
        checkpoint=torch.load('./train_EOG/disc/'+all[len(all)-1])
...
for epoch in range(NUM_EPOCHS):
    ...
    with torch.no_grad():
        ...
        niter = epoch
        writer_real = SummaryWriter(f"EOG_logs")
...
############################################################
    ...
    if epoch%5==0:
        ...
        torch.save(state_gen,'./train_EOG/gen/epoch_{}_gen.pth'.format(epochpoint))
        torch.save(state_disc,'./train_EOG/disc/epoch_{}_disc.pth'.format(epochpoint))
